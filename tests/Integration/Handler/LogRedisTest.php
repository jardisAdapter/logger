...
class LogRedisTest extends TestCase
{
    ...
    public function testLogToRedis(): void
    {
        $logger = (new LoggerBuilder('TestContext'))
            ->addHandler(new LogRedis(LogLevel::INFO, $this->redis, ttl: 300))
            ->getLogger();
    ...
    {
        $logger = (new LoggerBuilder('TestContext'))
            ->addHandler(new LogRedis(LogLevel::INFO, $this->redis, ttl: 2))
            ->getLogger();
    ...
    {
        $logger = (new LoggerBuilder('TestContext'))
            ->addHandler(new LogRedis(LogLevel::DEBUG, $this->redis, ttl: 300))
            ->getLogger();

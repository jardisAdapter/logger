...
class LogWebhookTest extends TestCase
{
    ...
    public function testSetHeaderInvalidatesTransportAndLogsCorrectly(): void
    {
        $webhook = new LogWebhook(
            LogLevel::INFO,
            'https://example.com/webhook'
        ...
        // Log before setHeader
        $logger->info('Before header change');

        // Change header (invalidates internal transport cache)
        $webhook->setHeader('X-New-Header', 'new-value');

        // Log after setHeader — must still work correctly
        $logger->info('After header change');

        rewind($stream);
        $output = stream_get_contents($stream);

        $this->assertStringContainsString('Before header change', $output);
        $this->assertStringContainsString('After header change', $output);
        $this->assertEquals('new-value', $webhook->getHeaders()['X-New-Header']);
    }
}
